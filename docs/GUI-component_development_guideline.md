...
Webで調べてみると、アトミックデザインの採用をやめたという記事がいくつも英語でも日本語でも見つかります。例えば「Why so many Design Systems using Atomic Design fail to succeed?」という記事がすぐに見つかりました。
...
まずUIコンポーネントの分類を見直してみます。

UIコンポーネントの分類を見直すことで、コンポーネント間の役割や使い方がより明確になり、開発が効率的になると考えたためです。
...
- **ビューコンポーネント**はデータの表示、**パーツコンポーネント**は見た目や動きに特化。
- シンプルな理解しやすいコンポーネントを作成する。
- 各コンポーネントが持つ責任を明確にし、必要な部分を分けて作成する。

## 設計の手順

1. **ビューコンポーネントの抽出**
   - まず表示対象のページに相当するビューコンポーネントを抽出します。
...
5. **抽出後のパーツの分類**
   - ビューコンポーネントの抽出が完了したら、各ビューコンポーネント内で**配置と装飾（テンプレート）**、**表示ギミック**、**基礎的なパーツ**に分類します。フレームワークが提供しているコンポーネントでも最低限のプロパティ指定で使えるのであればそのまま使ってもかまいません。
...
この例で使われているコンポーネントは、本記事で示したコンポーネントの分類に当てはめると次のようになります。
